"use strict";

const encryptedExtensions = {
  mgg: { ext: "ogg", version: "v2" },
  mgg1: { ext: "ogg", version: "v2" },
  mflac: { ext: "flac", version: "v2" },
  mflac0: { ext: "flac", version: "v2" },
  qmcflac: { ext: "flac", version: "v2" },
  qmcogg: { ext: "ogg", version: "v2" },
  qmc0: { ext: "mp3", version: "v1" },
  qmc2: { ext: "ogg", version: "v1" },
  qmc3: { ext: "mp3", version: "v1" },
  bkcmp3: { ext: "mp3", version: "v1" },
  bkcflac: { ext: "flac", version: "v1" },
  tkm: { ext: "m4a", version: "v1" },
  "666c6163": { ext: "flac", version: "v1" },
  "6d7033": { ext: "mp3", version: "v1" },
  "6f6767": { ext: "ogg", version: "v1" },
  "6d3461": { ext: "m4a", version: "v1" },
  "776176": { ext: "wav", version: "v1" }
};

const plainAudioExtensions = new Set([
  "mp3",
  "flac",
  "m4a",
  "aac",
  "ogg",
  "opus",
  "wav",
  "aif",
  "aiff",
  "aifc",
  "caf"
]);

const outputFormats = ["original", "mp3", "flac", "m4a", "ogg", "wav"];
const preferredFormatDisplayOrder = ["MP3", "FLAC", "M4A", "AAC", "OGG", "OPUS", "WAV", "AIFF", "CAF"];

const privateKey256 = [
  
];

function extensionOf(filePath) {
  const match = /\.([^.\\/]+)$/.exec(filePath);
  return match ? match[1].toLowerCase() : "";
}

function isSupportedExtension(ext) {
  return plainAudioExtensions.has(ext) || Object.prototype.hasOwnProperty.call(encryptedExtensions, ext);
}

function displayAudioFormat(ext) {
  const lower = ext.toLowerCase();
  const encrypted = encryptedExtensions[lower];
  if (encrypted) return `${lower.toUpperCase()}->${encrypted.ext.toUpperCase()}`;
  if (["aif", "aiff", "aifc"].includes(lower)) return "AIFF";
  return lower.toUpperCase();
}

function formatBreakdown(filePaths) {
  const counts = new Map();
  for (const filePath of filePaths) {
    const format = displayAudioFormat(extensionOf(filePath));
    counts.set(format, (counts.get(format) || 0) + 1);
  }
  return [...counts.entries()]
    .sort((a, b) => {
      const ai = preferredFormatDisplayOrder.indexOf(a[0]);
      const bi = preferredFormatDisplayOrder.indexOf(b[0]);
      const ax = ai === -1 ? Number.MAX_SAFE_INTEGER : ai;
      const bx = bi === -1 ? Number.MAX_SAFE_INTEGER : bi;
      return ax === bx ? a[0].localeCompare(b[0]) : ax - bx;
    })
    .map(([format, count]) => `${format} ${count}`)
    .join("、");
}

module.exports = {
  encryptedExtensions,
  plainAudioExtensions,
  outputFormats,
  privateKey256,
  extensionOf,
  isSupportedExtension,
  displayAudioFormat,
  formatBreakdown
};
